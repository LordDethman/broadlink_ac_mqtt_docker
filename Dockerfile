...
RUN \
 ...
 AC2MQTT_RELEASE=$(curl -sX GET "https://api.github.com/repos/LordDethman/broadlink_ac_mqtt/releases/latest" \
 | jq -r '. | .tag_name'); \
 echo "Latest Release is ${AC2MQTT_RELEASE}" && \
 echo "Downloading Version ${AC2MQTT_RELEASE}" && \
 curl -o \
 /tmp/ac2mqtt.tar.gz -L \
	"https://github.com/LordDethman/broadlink_ac_mqtt/archive/${AC2MQTT_RELEASE}.tar.gz" && \
